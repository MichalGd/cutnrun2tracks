...
worker() {
    ...
    local work="${OUTPUT_DIR}/02_trimmed_fastq" raw1="${work}/${key}.R1.merged.fastq.gz"
    ...
    if is_true "$RUN_FASTQC"; then
        if [[ "$layout" == "PE" ]]; then
            run_logged fastqc --threads 1 --outdir "${OUTPUT_DIR}/01_fastq_qc/raw" "$raw1" "$raw2" >"$log" 2>&1
        else
            run_logged fastqc --threads 1 --outdir "${OUTPUT_DIR}/01_fastq_qc/raw" "$raw1" >"$log" 2>&1
        fi
    fi
    if is_true "$TRIM_ADAPTERS"; then
        ...
        if [[ "$layout" == "PE" ]]; then
            run_logged trim_galore --paired --length "$MIN_TRIMMED_LENGTH" --output_dir "$work" \
            ...
            run_logged trim_galore --length "$MIN_TRIMMED_LENGTH" --output_dir "$work" \
                "${extra[@]}" "$raw1" >>"$log" 2>&1
    ...
    if is_true "$RUN_FASTQC"; then
        ...
        if [[ "$layout" == "PE" ]]; then
            run_logged fastqc --threads 1 --outdir "${OUTPUT_DIR}/01_fastq_qc/trimmed" "$trimmed1" "$trimmed2" >>"$log" 2>&1
        else
            run_logged fastqc --threads 1 --outdir "${OUTPUT_DIR}/01_fastq_qc/trimmed" "$trimmed1" >>"$log" 2>&1
        fi
    fi
}
